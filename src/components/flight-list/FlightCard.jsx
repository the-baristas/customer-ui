...
const FlightCard = (props) => {
    const handleClick = (event, seatClass) => {
        props.onFlightSelection(props.flight, seatClass);
    };

    const duration = moment.duration(
        moment(props.flight.arrivalTime).diff(props.flight.departureTime)
    );
            ...
            <Row className="flight-card">
                <Col xs={12} lg={2}>
                    <Row className="origin-airport">
                        {props.flight.route.originAirport.city} (
                        {props.flight.route.originAirport.iataId})
                    </Row>
                    <Row className="departure-time">
                        {moment(props.flight.departureTime).format("h:mm a")}
                    </Row>
                    <Row className="departure-date">
                        {moment(props.flight.departureTime).format(
                            "MMMM Do, YYYY"
                        )}
                    </Row>
                </Col>
                ...
                <Col xs={12} lg={2}>
                    ...
                    <Row className="arrival-date">
                        {moment(props.flight.arrivalTime).format(
                            "MMMM Do, YYYY"
                        )}
                    </Row>
                </Col>
                ...
                <Col xs={12} lg={2}>
                    <Row className="b-e">Economy</Row>
                    <Row className="book-rc">
                        <Button
                            className="book-button"
                            onClick={(event) =>
                                handleClick(event, SeatClass.ECONOMY)
                            }
                        >
                            ${props.flight.economyPrice}
                        </Button>
                    </Row>
                </Col>

                <Col xs={12} lg={2}>
                    <Row className="b-b">Business</Row>
                    <Row className="book-rc">
                        <Button
                            className="book-button"
                            onClick={(event) =>
                                handleClick(event, SeatClass.BUSINESS)
                            }
                        >
                            ${props.flight.businessPrice}
                        </Button>
                    </Row>
                </Col>

                <Col xs={12} lg={2}>
                    <Row className="b-f">First</Row>
                    <Row className="book-rc">
                        <Button
                            className="book-button"
                            onClick={(e) => handleClick(e, SeatClass.FIRST)}
                        >
                            ${props.flight.firstPrice}
                        </Button>
                    </Row>
                </Col>
            </Row>
